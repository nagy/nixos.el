...
(ert-deftest nixos-flake-metadata-fields ()
  ...
  (let* ((meta (make-hash-table :test 'equal))
         ...
         (mk (lambda (type &rest kvs)
              (let ((n (make-hash-table :test 'equal))
                    (lk (make-hash-table :test 'equal))
                    (og (make-hash-table :test 'equal)))
                (puthash "type" type lk)
                (puthash "type" type og)
                (while kvs
                  (puthash (car kvs) (cadr kvs) lk)
                  (setq kvs (cddr kvs)))
                (puthash "locked" lk n)
                (puthash "original" og n)
                n)))
         (root-i (make-hash-table :test 'equal))
         (root (funcall mk "git"))
         (crane (funcall mk "github" "owner" "ipetkov" "repo" "crane" "rev" "abc"))
         (nixpkgs (funcall mk "github" "owner" "NixOS" "repo" "nixpkgs" "rev" "def")))
    (puthash "inputs" root-i root)
    (puthash "crane" "crane" root-i)
    (puthash "nixpkgs" "nixpkgs" root-i)
    (puthash "root" root nodes)
    (puthash "crane" crane nodes)
    (puthash "nixpkgs" nixpkgs nodes)
    (puthash "root" "root" locks)
    ...
    (let ((fields (nixos--flake-metadata-fields meta)))
      ...
      (let ((inputs (cdr (assoc "Inputs:" fields))))
        (should (listp inputs))
        (should (equal (car inputs) "├───crane: github:ipetkov/crane/abc"))
        (should (equal (car (last inputs)) "└───nixpkgs: github:NixOS/nixpkgs/def"))))))
